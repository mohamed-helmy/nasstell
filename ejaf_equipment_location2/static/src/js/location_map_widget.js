odoo.define('ejaf_equipment_location.location_map_widget', function(require){

    var basic_fields = require('web.basic_fields');
    var registry = require('web.field_registry');
    var GoogleMapWidget = require('ejaf_equipment_location.google_map_widget');
    ...
    registry.add('request_location_map', location_map_widget);

    var equipment_location_map_widget = basic_fields.FieldChar.extend({
        init: function(parent, name, record, options){
            this._super.apply(this, arguments);
            this.lat = 0.0;
            this.lng = 0.0;
            ...
            geocoder.geocode({'location': latLng}, function (results, status) {
                if (status === 'OK') {
                    if (self.$input) {
                        self.$input.val(results[0].formatted_address);
                        var changes ={
                            'equipment_latitude':lat,
                            'equipment_longitude':lng,
                        };
                        self._onUpdateWidgetFields(changes);
                        self._doAction();
                    }
    ...
    registry.add('location_map', equipment_location_map_widget);
});