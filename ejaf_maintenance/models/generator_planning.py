...
class GeneratorPlanning(models.Model):
    _name = 'generator.planning'
    _rec_name = 'maintenance_team_id'
    _description = 'Generator Planning'

    def _get_generator_teams(self):
        generator_type = self.env['maintenance.type'].sudo().search([('name', '=', 'preventive')], limit=1)
        generator_tag = self.env['maintenance.tag'].sudo().search([('name', '=', 'generators')], limit=1)
        teams = self.env['maintenance.team'].sudo().search(
            [('maintenance_type_ids', 'in', [generator_type.id]), ('maintenance_tag_ids', 'in', [generator_tag.id]),
             ('company_id', '=', self.env.company.id)])
        return [('id', 'in', teams.ids)]

    site_id = fields.Many2one(comodel_name="maintenance.equipment", required=True, ondelete="cascade")
    ...
    def onchange_site_id(self):
        for planning in self:
            planning.maintenance_team_id = planning.site_id.maintenance_team_id if planning.site_id and planning.site_id.maintenance_team_id else False
            planning.generator_ids = planning.site_id.generator_ids.ids if planning.site_id and planning.site_id.generator_ids else False
...
class GeneratorPlanningLine(models.Model):
    _name = 'generator.planning.line'
    _description = 'Generator Planning Line'
    _rec_name = 'plan_id'
    ...
    def unlink(self):
        ...
        return super(GeneratorPlanningLine, self).unlink()

    @api.model
    def _cron_generate_requests_send_notifications(self):
        subtype_commit = self.env.ref("mail.mt_comment")
        planning_lines = self.env['generator.planning.line'].search(
            [('date', '>=', fields.Date.today()), ('maintenance_request_id', '=', False)]).filtered(
            lambda x: (x.date - fields.Date.today()).days <= self.env.company.generator_planning_notify_period)
    ...
    def create_new_request(self, equipment, date, planning_line):
        ...
        return self.env['maintenance.request'].create({
            'name': _('Preventive Generator Maintenance - %s') % equipment.name,
            'origin': 'Preventive Generator Maintenance',
            'request_date': date,
            ...
            'maintenance_team_id': planning_line.maintenance_team_id.id if planning_line.maintenance_team_id else planning_line.plan_id.maintenance_team_id.id,
            'duration': equipment.maintenance_duration,
            'company_id': equipment.company_id.id or self.env.company.id,
            'planned_generator_ids': planning_line.planned_generator_ids.ids
        })
