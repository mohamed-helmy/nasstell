...
class JobOrder(models.Model):
    ...
    def _default_picking_type_id(self):
        ...
        picking_type = self.env['stock.picking.type'].search([('company_id', '=', company), ('code', '=', 'outgoing')],
                                                             limit=1)
        return picking_type.id

    name = fields.Char(string="Job Order NO", required=True,
                       default='/',
    ...
    delivery_ids = fields.One2many(comodel_name="stock.picking", inverse_name="job_order_id",
                                   string="Delivery Orders")
    spare_part_ids = fields.One2many(comodel_name="spare.part", inverse_name="job_order_id",
                                     string="Spare Part")
    ...
    technical_inspection_ids = fields.One2many(comodel_name="technical.inspection",
    ...
    def _compute_technical_inspection_count(self):
        for record in self:
            if record.technical_inspection_ids:
                record.technical_inspection_count = len(record.technical_inspection_ids) or 0.0
            else:
                record.technical_inspection_count = 0.0

    @api.depends('timesheet_ids', 'timesheet_ids.cost')
    def _compute_total_cost(self):
        for record in self:
            if record.timesheet_ids:
                record.total_cost = sum([line.cost for line in record.timesheet_ids]) or 0.0
            else:
    ...
    def create(self, values):
        ...
        return super(JobOrder, self).create(values)
